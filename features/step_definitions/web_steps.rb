...
Given /^I am in the event "([^"]*)" (.+)$/ do |event_name, page_name|
  @event_id = Event.find_by_name(event_name).id.to_s()
  visit path_to(page_name)
end

Given /^the user "([^"]*)" exists$/ do |user_name|
  user          = User.new
  user.name     = user_name
  user.password = "1234"
  user.email    = "a_mail"
...
Given /^the user "([^"]*)" with password "([^"]*)" exists$/ do |user_name, user_pass|
  user          = User.new
  user.name     = user_name
...
Given /^the "([^"]*)" of "([^"]*)" exists for "([^"]*)"$/ do |event_name, user_name, event_date|
  current        = User.find_by_name(user_name)
  event          = Event.new
  event.name     = event_name
  event.date     = event_date
  event.username = current.name
  event.user_id  = current.id
  event.save
end

Given /^I am in the user "([^"]*)" (.+)$/ do |user_name, page_name|
  @user_id = User.find_by_name(user_name).id.to_s()
  visit path_to(page_name)
end
